...
async def _ingest_schedule(settings: Settings, seasons: Sequence[int]) -> int:
    """Load the game schedule, which strength of schedule reads."""
    from fhe.data.ingest.nflverse_schedule import ingest_schedule
    from fhe.data.providers.nflverse import NflverseProvider
    from fhe.db import create_engine, create_session_factory
    ...
    try:
        async with NflverseProvider(settings) as nflverse:
            run = await ingest_schedule(factory, nflverse, list(seasons))
        print(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    schedule = ingest_sub.add_parser(
        "schedule", help="Load the game schedule, for strength of schedule"
    )
